    ...
    for c in cl:
        if c.addr == addr:
            if 'readings' in payload:
                c.write_message(
                    gen_topic_message(msg.topic, message=payload['readings'][0]))
            else:
                c.write_message(
                    gen_topic_message(msg.topic, message=payload))
...
class IndexHandler(web.RequestHandler):
    def get(self):
        self.render("demo_" + DEMO_APP + ".html")

class DemoHandler(web.RequestHandler):
    def get(self, path):
        self.render("demo_" + path + ".html")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--case', '-c',
        ...
        choices=["cam", "acrn", "js", "simplejs", "charger"])
    args = parser.parse_args()

    DEMO_APP = args.case
    print("Running case: " + DEMO_APP)
